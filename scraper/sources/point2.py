"""Point2Homes scraper — typically low bot resistance, good coverage."""
import re
...
class Point2Scraper(BaseScraper):
    ...
    def search_url(self, zip_code: str, page: int) -> str:
        return (
            f"https://www.point2homes.com/US/Real-Estate-Listings/CA/Irvine.html"
            f"?BedMin=3&BathMin=2&SqFtMin=1000&PropertyType=Single+Family&Zip={zip_code}"
            f"&p={page}"
        )
    ...
    def parse_search_page(self, html: str, zip_code: str) -> List[Listing]:
        ...
        if not listings:
            soup = self.soup(html)
            cards = soup.select(".listing-details-prop, .search-listing-result, [data-listingid]")
            for card in cards:
    ...
    def _from_card(self, card, zip_code: str) -> Optional[Listing]:
        try:
            ...
            addr_el = card.select_one(".address, .listing-address, h2, h3")
            l.address = addr_el.get_text(strip=True) if addr_el else ""
            link = card.select_one("a[href]")
            href = link["href"] if link else ""
            l.listing_url = href if href.startswith("http") else f"https://www.point2homes.com{href}"
