...
class Century21Scraper(BaseScraper):
    ...
    def search_url(self, zip_code: str, page: int) -> str:
        return (
            f"https://www.century21.com/real-estate/irvine-ca/LCAR-Q14286/"
            f"?beds=3&baths=2&sqftMin=1000&types=SFR&zip={zip_code}&pg={page}"
        )
    ...
    def parse_search_page(self, html: str, zip_code: str) -> List[Listing]:
        ...
        if not listings:
            soup = self.soup(html)
            cards = soup.select(".property-container, .listing-item, .home-card")
            for card in cards:
    ...
    def _from_card(self, card, zip_code: str) -> Optional[Listing]:
        try:
            ...
            addr_el = card.select_one(".property-address, .listing-address")
            l.address = addr_el.get_text(strip=True) if addr_el else ""
            link = card.select_one("a[href]")
            href = link["href"] if link else ""
            l.listing_url = href if href.startswith("http") else f"https://www.century21.com{href}"
