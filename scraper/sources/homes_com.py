...
class HomesComScraper(BaseScraper):
    source_name = "homes_com"
    page_size = 20

    def search_url(self, zip_code: str, page: int) -> str:
        return (
            f"https://www.homes.com/homes-for-sale/{zip_code}/"
            f"?beds=3-&baths=2-&sqft=1000-&type=single-family&p={page}"
        )
    ...
    def parse_search_page(self, html: str, zip_code: str) -> List[Listing]:
        ...
        if not listings:
            soup = self.soup(html)
            cards = soup.select("[data-testid='listing-card'], .listing-card, .property-card-listing")
            for card in cards:
    ...
    def _from_card(self, card, zip_code: str) -> Optional[Listing]:
        try:
            ...
            addr_el = card.select_one("[data-testid='address'], .listing-address, address")
            l.address = addr_el.get_text(strip=True) if addr_el else ""
            link = card.select_one("a[href]")
            href = link["href"] if link else ""
            l.listing_url = href if href.startswith("http") else f"https://www.homes.com{href}"
